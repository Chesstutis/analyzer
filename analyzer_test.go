...
func BenchmarkAnalyzeEvergreenGameBlack(b *testing.B) {
	benchmarkAnalyzeGame(b, evergreenGame, chess.Black)
}

func BenchmarkAnalyzeOperaGameBlack(b *testing.B) {
	benchmarkAnalyzeGame(b, operaGame, chess.Black)
}

func benchmarkAnalyzeGame(b *testing.B, pgnText string, color chess.Color) {
	b.Helper()

	pgn, err := chess.PGN(strings.NewReader(pgnText))
	if err != nil {
		b.Fatalf("PGN parsing failed: %v", err)
	}
	game := chess.NewGame(pgn)

	eng, err := uci.New("stockfish")
	if err != nil {
		b.Fatalf("stockfish startup failed: %v", err)
	}
	defer eng.Close()

	a, err := NewAnalyzer(eng)
	if err != nil {
		b.Fatalf("analyzer creation failed: %v", err)
	}

	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := a.AnalyzeGame(game, color); err != nil {
			b.Fatalf("AnalyzeGame failed: %v", err)
		}
	}
}

func BenchmarkAnalyzeConfigs(b *testing.B) {
	referenceCfg := Config{
		Threads:            4,
		HashMB:             1024,
		BestMoveDepth:      14,
	...
	candidateCfg := []struct {
		...
		{"verify25ms", Config{Threads: 2, HashMB: 512, BestMoveDepth: 10, VerifyMoveTime: 25 * time.Millisecond, BlunderThresholdCP: 200, SkipOpeningPlies: 8}},
		{"verify50ms", Config{Threads: 2, HashMB: 512, BestMoveDepth: 10, VerifyMoveTime: 50 * time.Millisecond, BlunderThresholdCP: 200, SkipOpeningPlies: 8}},
		{"verify75ms", Config{Threads: 2, HashMB: 512, BestMoveDepth: 10, VerifyMoveTime: 75 * time.Millisecond, BlunderThresholdCP: 200, SkipOpeningPlies: 8}},
		{"verify100ms", Config{Threads: 2, HashMB: 512, BestMoveDepth: 10, VerifyMoveTime: 100 * time.Millisecond, BlunderThresholdCP: 200, SkipOpeningPlies: 8}},
	}
}
